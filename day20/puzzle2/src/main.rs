...
fn main() {
    ...
    let mut seen = modules.iter().fold(HashMap::new(), |mut acc, m| {
        if m.outputs.contains(&feed) {
            acc.insert(m.name.clone(), 0);
        }
        acc
    });
    ...
    loop {
        ...
        while !q.is_empty() {
            ...
            if module.name == feed && pulse == "hi" {
                let seen_n = *seen.get(&origin).unwrap();
                seen.insert(origin.clone(), seen_n + 1);
                if cycle_lengths.get(&origin).is_none() {
                    cycle_lengths.insert(origin.clone(), presses);
                } else {
                    assert!(
                        presses == seen.get(&origin).unwrap() * cycle_lengths.get(&origin).unwrap()
                    );
                }

                if seen.values().all(|x| x > &0) {
                    let sum = cycle_lengths.values().fold(1, |acc, x| lcm(acc, *x));
                    println!("Sum: {:?}", sum);
                    exit(0);
                }
            }
